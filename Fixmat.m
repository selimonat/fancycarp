classdef Fixmat < Project
    properties (Hidden,Constant)         
        dummy      = make_gaussian2D(Fixmat.PixelPerDegree*2.65,Fixmat.PixelPerDegree*2.65,Fixmat.PixelPerDegree,Fixmat.PixelPerDegree,Fixmat.PixelPerDegree*2.65/2,Fixmat.PixelPerDegree*2.65/2);
...
%         kernel = Fixmat.dummy;
    ...
    methods 
        function obj = Fixmat(subjects,runs)            
            ...
            obj       = obj.FilterFixmat('phase',runs);
            %% remove fixations outside of the image border            
            selection = ~(obj.x <= obj.rect(3) | obj.x > obj.rect(4) | obj.y <= obj.rect(1) | obj.y > obj.rect(2));
            obj       = obj.ApplySelection(selection);
        end
        function out=FilterFixmat(self,varargin)
            %takes VARARGIN pairs to filter fixmat
            selection = zeros(1,length(self.subject));
            for n = 1:2:length(varargin)
                selection = selection + ismember(self.(varargin{n}),varargin{n+1});
            end
            out = self.ApplySelection(selection);
        end        
        function [out]=ApplySelection(self,selection)
    ...
    methods
        function maps = plot(self,varargin)            
            c  = 0;
            for v = varargin
                c = c+1;
                out    = self.FilterFixmat(v{1}{:});
                x      = round(out.x);
                y      = round(out.y);
                FixMap = accumarray([y(:) x(:)],1,[out.rect(2) out.rect(4)]);
                FixMap = FixMap./sum(FixMap(:));
                FixMap = conv2(sum(self.kernel),sum(self.kernel,2),FixMap,'same');                
                maps(:,:,c) = FixMap(self.rect(2)/2-250 : self.rect(2)/2+250, self.rect(4)/2-250 : self.rect(4)/2+250);
            end
            [d u] = GetColorMapLimits(maps,9);
            for nc = 1:size(maps,3)
                figure(nc);
                h = imagesc(maps(:,:,nc),[0 u]);
                colorbar;
                set(h,'alphaData',Scale(maps(:,:,nc)));
            end
        end
        function fixmat = getfixmat(self,edfdata, edfmeta)
